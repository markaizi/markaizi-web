...
export default function AdminProfilPage() {
  ...
  async function handleName(e: React.FormEvent) {
    ...
    const res  = await fetch("/api/musteri/admin/profile", {
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      <main className="max-w-[520px] mx-auto px-6 py-10 space-y-6">
        <h1 className="font-black text-[22px] text-white">Admin Profili</h1>
        ...
        <form onSubmit={handleName} className="rounded-2xl p-5 space-y-4"
          ...
          <div>
            <label htmlFor="admin-profil-name" className="block text-[11px] font-semibold text-[#8a8a9a] uppercase tracking-wide mb-1.5">Yeni Ad Soyad</label>
            <input id="admin-profil-name" value={nameForm.name} onChange={(e) => setNameForm((f) => ({ ...f, name: e.target.value }))}
          ...
          <div>
            <label htmlFor="admin-profil-email" className="block text-[11px] font-semibold text-[#8a8a9a] uppercase tracking-wide mb-1.5">Yeni E-posta</label>
            <input id="admin-profil-email" type="email" value={nameForm.email} onChange={(e) => setNameForm((f) => ({ ...f, email: e.target.value }))}
        ...
        <form onSubmit={handlePw} className="rounded-2xl p-5 space-y-4"
          ...
          {[
            ...
            <div key={key}>
              <label htmlFor={`admin-profil-${key}`} className="block text-[11px] font-semibold text-[#8a8a9a] uppercase tracking-wide mb-1.5">{label}</label>
              <input id={`admin-profil-${key}`} required type="password" value={pwForm[key as keyof typeof pwForm]}
