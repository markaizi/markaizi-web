...
export default function CalisanProfilPage() {
  ...
  async function handleName(e: React.FormEvent) {
    ...
    const res  = await fetch("/api/musteri/calisan/profile", {
    ...
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      <main className="max-w-[520px] mx-auto px-6 py-10 space-y-6">
        ...
        <form onSubmit={handleName} className="rounded-2xl p-5 space-y-4"
          ...
          <div>
            <label htmlFor="calisan-profil-name" className="block text-[11px] font-semibold text-[#8a8a9a] uppercase tracking-wide mb-1.5">Yeni Ad Soyad</label>
            <input id="calisan-profil-name" value={nameForm.name} onChange={(e) => setNameForm((f) => ({ ...f, name: e.target.value }))}
          ...
          <div>
            <label htmlFor="calisan-profil-email" className="block text-[11px] font-semibold text-[#8a8a9a] uppercase tracking-wide mb-1.5">Yeni E-posta</label>
            <input id="calisan-profil-email" type="email" value={nameForm.email} onChange={(e) => setNameForm((f) => ({ ...f, email: e.target.value }))}
        ...
        <form onSubmit={handlePw} className="rounded-2xl p-5 space-y-4"
          ...
          {[
            ...
            <div key={key}>
              <label htmlFor={`calisan-profil-${key}`} className="block text-[11px] font-semibold text-[#8a8a9a] uppercase tracking-wide mb-1.5">{label}</label>
              <input id={`calisan-profil-${key}`} required type="password" value={pwForm[key as keyof typeof pwForm]}
