...
interface NotificationItem {
  id: string;
  type: "WORKLOG_PRICED" | "ADMIN_MESSAGE";
  title: string;
  body: string;
  readAt: string | null;
  createdAt: string;
...
export default function StaffNotificationBell() {
  ...
  async function load() {
    setLoading(true);
    const res = await fetch("/api/musteri/calisan/notifications");
    if (res.ok) {
  ...
  async function markRead(id: string) {
    ...
    await fetch(`/api/musteri/calisan/notifications/${id}`, {
      method: "PATCH",
  ...
  async function markAllRead() {
    ...
    setUnreadCount(0);
    await fetch("/api/musteri/calisan/notifications/read-all", { method: "PATCH" });
  }
